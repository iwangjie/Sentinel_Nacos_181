...
public class SentinelCollector extends Collector {
    ...
    public List<MetricFamilySamples> collect() {
        ...
        try {
            ...
            GaugeMetricFamily metricFamily = new GaugeMetricFamily(
                    "sentinel_metrics", // 通用的指标名称
                    MetricConstants.METRIC_HELP,
                    Arrays.asList("app", MetricConstants.RESOURCE, MetricConstants.CLASSIFICATION, MetricConstants.METRIC_TYPE)
            );
            ...
            for (MetricNode node : nodes) {
                ...
                for (String type : types) {
                    ...
                    metricFamily.addMetric(
                            Arrays.asList(appName, node.getResource(), String.valueOf(node.getClassification()), type),
                            val,
                            recordTime
                    );
                }
